...
struct InstructionBuffer *vm_init_instruction_buffer(void) {
...
void vm_insert_instruction_buffer(struct InstructionBuffer *instruction_buffer, const uint8_t instruction) {
  if (instruction_buffer->used == instruction_buffer->size) {
    ...
    instruction_buffer->buffer = new_buffer;
